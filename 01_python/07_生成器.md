...
#### 迭代器
相对于可迭代对象,实现了 __iter__()和__next()__ 方法,它不会一次把所有内容都加载到内存中,只有需要时才返回结果. 可以通过一定的方式将可迭对象转化成迭代器
...
恢复执行并向生成器函数“发送”一个值。**value 参数将成为当前 yield 表达式的结果**。 send() 方法会返回生成器所产生的下一个值，或者如果生成器没有产生下一个值就退出则会引发 StopIteration。 
