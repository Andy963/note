...
卷积--- 解决问题
- 局部连接(基于图像的区域性)
- 参数共享（基于图像的特征与位置无关的特性）

卷积操作相当于 卷积核在输入图像上滑动 
输出size = 输入size - 卷积核size + 1

为了使输出size 不变， 通常使用padding 
...
将上一层输出展开（一维数组）并连接到每一个神经元上。 


#### 神经网络训练优化

随机梯度下降：
每次只使用一个样本(相当于每次参与训练的只有一个样本，一个样本不能反映整个数据集的梯度的方向，会导致收敛速度比较慢)

Mini-Batch梯度下降：
每次使用小部分数据进行训练，这部分数据是随机采样出来的，可以代表整个数据集的方向

梯度下降存在局部极值和鞍点（saddle point）的问题， 由此提出动量梯度下降的方法
